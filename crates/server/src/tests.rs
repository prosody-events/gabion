...
fn maps_descriptors_and_returns_ok_or_over_limit() {
    run_local(async {
        ...
        let first = harness
            ...
            .await
            .expect("evaluate first request");
        let second = harness
            .limiter
            .should_rate_limit_at(request, 1)
            .await
            .expect("evaluate second request");
        ...
        for tick in 2..10 {
            ...
            harness
                ...
                .await
                .expect("evaluate request while filling limit");
        }
        ...
        let response = harness
            .limiter
            .should_rate_limit_at(over, 11)
            .await
            .expect("evaluate over-limit request");
        assert_eq!(
    ...
    run_local(async {
        ...
        for tick in 1..10 {
            ...
            assert_eq!(
                harness
                    ...
                    .await
                    .expect("evaluate rejected request")
                    .overall_code,
                rate_limit_response::Code::OverLimit as i32
            );
        }
        ...
        assert_eq!(
            harness
                ...
                .await
                .expect("evaluate recovered request")
                .overall_code,
            rate_limit_response::Code::Ok as i32
        );
...
fn rejects_requests_over_cardinality_limits_before_recording() {
    run_local(async {
        ...
        let response = harness
            .limiter
            .should_rate_limit_at(request, 0)
            .await
            .expect("evaluate cardinality request");
...
fn quickcheck_response_shape_matches_descriptor_shape(case: EnvoyRequestCase) -> TestResult {
    run_local(async move {
        ...
        let response = harness
            ...
            .should_rate_limit_at(request, 0)
            .await
            .expect("evaluate quickcheck request");
