...
use crate::identity::{derive_identity, random_seed};
...
fn falls_back_to_hostname_consistently() {
    ...
    // or random). Exact value is machine-dependent, but two
...
#[test]
fn incarnation_is_unix_seconds_with_floor() {
    let id = derive_identity(Some("incarnation-check"));
    let inc = id.incarnation;
